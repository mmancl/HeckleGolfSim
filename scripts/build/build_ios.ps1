...
if ($IsMac) {
    ...
    $shScript = Join-Path $RepoRoot "scripts\build\build_ios.sh"
...
# Record previous run ID to prevent race conditions
$prevRunId = & gh run list --repo $repoSlug --workflow=build_ios.yml --limit 1 --json databaseId --jq ".[0].databaseId" 2>$null
if (-not $prevRunId) { $prevRunId = "0" }
...
Write-Host "[2/3] Waiting for new macOS runner job to be scheduled..." -ForegroundColor Yellow

$runId = ""
$attempts = 0
while (-not $runId -and $attempts -lt 30) {
    Start-Sleep -Seconds 3
    $latestRun = & gh run list --repo $repoSlug --workflow=build_ios.yml --limit 1 --json databaseId,status --jq ".[0]" | ConvertFrom-Json
    if ($latestRun -and $latestRun.databaseId -and ($latestRun.databaseId -ne $prevRunId)) {
...
if (-not $runId) {
    throw "Could not retrieve the newly triggered workflow run ID from GitHub (waited 90s)."
}
...
if ($runStatus -ne "success") {
    Write-Host ""
    Write-Host "[ERROR] Cloud iOS Build Failed! Showing failure logs:" -ForegroundColor Red
    Write-Host "-------------------------------------------------------" -ForegroundColor Red
    & gh run view $runId --repo $repoSlug --log-failed
    Write-Host "-------------------------------------------------------" -ForegroundColor Red
    throw "GitHub Actions iOS build failed with status: $runStatus. See logs above for details."
}
